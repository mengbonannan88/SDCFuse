...
class llvip(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        vis_clip_image = Image.open(os.path.join(self.vis_rain, name)).resize((224,224))

        #####处理信息熵选择权重

        inf_image = self.transform(inf_image)
        vis_rain = self.transform(vis_rain)
        vis_clip_image = self.transform(vis_clip_image)


        vis_rain_y, vis_cb_image, vis_cr_image = RGB2YCrCb(vis_rain)

        return vis_rain_y, vis_cb_image, vis_cr_image, inf_image, vis_clip_image, name
