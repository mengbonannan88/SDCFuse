...
class Cross_AttentionBase(nn.Module):
    def __init__(self,
                 dim,
                 num_heads=8,
                 qkv_bias=False, ):
        super(Cross_AttentionBase, self).__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.qkv1 = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=qkv_bias)
    ...
    def forward(self, x, y):
        ...
        #-----------------------------------------------------------------------#
        #---------------------开始第一个模态和第二个模态的混合注意力-------------------#
        attn = (q @ k_y.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        ...
        out_y = rearrange(out_y, 'b head c (h w) -> b (head c) h w',
                        head=self.num_heads, h=h, w=w)

        out_y = self.proj(out_y)
        #--------------------------------------------------------------------------#
        return out + out_y
...
class BaseFeatureExtraction(nn.Module):
    def __init__(self,
        ...
        self.attn = Cross_AttentionBase(dim, num_heads=num_heads, qkv_bias=qkv_bias, )
        self.norm2 = LayerNorm(dim, 'WithBias')
        self.mlp = Mlp(in_features=dim,
                       ffn_expansion_factor=ffn_expansion_factor, )

    def forward(self, x, y):
        out = x + y + self.attn(self.norm1(x), self.norm1(y))
...
if __name__ == '__main__':
    model = BaseFeatureExtraction(dim=64).cuda()
    img = torch.randn(2,64,8,8).cuda()
